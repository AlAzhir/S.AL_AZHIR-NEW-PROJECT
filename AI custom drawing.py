...
class DrawingClassifier:
    ...
    def classes_prompt(self):    
        ...
        self.proj_name=simpledialog.askstring("Project Name","Please enter your project name done below!",parent=msg)
        if os.path.exists(self.proj_name):
            ...
            self.class2=simpledialog.askstring("Class 1","What is the second class called?",parent=msg)
            self.class3=simpledialog.askstring("Class 1","What is the third class called?",parent=msg)    
    ...
    def init_gui(self):
        ...
        self.root=Tk()
        self.root.title(f="S.AL AZHIR Drawing Classifier Alpha v0.2-{self.proj_name}")
        ...
        save_btn=Button(btn_frame,text="Savve Model",command=self.save_model)
        ...
        self.status.label.grid(row=4,column=1,sticky=W+E)
    ...
    def train_model(self):
        ...
        for x in range(1,self.class3_counter):
            ...
            tkinter.messagebox.showinfo("S.AL AZHIR Drawing Classifieer","Model successfully trained!",parent=self.root)
    ...
    def predict(self):
        ...
        if prediction[0]==1:
            tkinter.messagebox.showinfo("S.AL AZHIR Drawing Classifier",f"The drawing is probablyn a{self.class1}",parent=self.root)
        elif prediction[0]==2:
            tkinter.messagebox.showinfo("S.AL AZHIR Drawing Classifier",f"The drawing is probablyn a{self.class2}",parent=self.root)   
        elif prediction[0]==3:
            tkinter.messagebox.showinfo("S.AL AZHIR Drawing Classifier",f"The drawing is probablyn a{self.class3}",parent=self.root)   
    def rotate_model(self):
        ...
        self.staus_lable.config(text=f"Current Model:{type(self.clf).__name__}")
       
    def save(self):
        file_path=filedialog.asksaveasfilename(defaultextension="pickle")
        with open(file_path,"wb") as f: 
            pickle.dump(self.clf,f)
            tkinter.messagebox.shoeinfo("S.AL AZHIR Drawing Classifier","Model successfully saved!",parent=self.root)

    def load_model(self):
        file_path=filedialog.askopenfilename()
        with open(file_path,"rb") as f:
            self.clf=pickle.load(f)
            tkinter.messagebox.showinfo("S.AL AZHIR Drawing Classifier","Model successfully loaded!",parent=self.root)
    ...
    def save_everything(self):
        ...
        with open(f"{self.proj_name}/{self.proj_name}_data.pickle","wb") as f:
            pickle.dump(data,f)
            tkinter.messagebox.showinfo("S.AL AZHIR Drawing Classifier","Project successfully saved!",parent=self.root)
