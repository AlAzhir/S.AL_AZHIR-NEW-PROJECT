...
frozen_model = 'frozen_interference_graph.pb'  # Corrected the filename
model = cv2.dnn_DetectionModel_create(frozen_model, config_file)
...
if not cap.isOpened():
    cap = cv2.VideoCapture(0)  # Corrected line to capture from the camera
if not cap.isOpened():
...
while True:
    ...
    cv2.imshow('object by simplilearn', frame)
